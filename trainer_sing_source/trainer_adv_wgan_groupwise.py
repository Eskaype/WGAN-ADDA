...
class wgan_trainer(object):
    ...
    def update_weights(self, input_, src_labels, target, tgt_labels, lamda_g_cup, lamda_g_disc, beta_cup, beta_disc, gamma, trainmodel):
        ...
        disc_clf, _ = self.disc_model(discriminator_x)
        import pdb
        ...
        for i,lab in enumerate(['disc', 'cup']):
            ...
            dist_s, dist_targ= self.wsn.update_wasserstein(D_src_out_classes, D_tar_out_classes, src_labels[:, i, :, :], targ_out[:, i, :, :].detach())
            if lab == 'cup':
                wasdist_src_cup+=dist_s
                wasdist_targ_cup+=dist_targ
            else:
                wasdist_src_disc+=dist_s
                wasdist_targ_disc+=dist_targ
        if trainmodel == 'train_gen':
            loss_seg = seg_loss  - lamda_g_cup * (wasdist_src_cup - wasdist_targ_cup) - lamda_g_disc * (wasdist_src_disc - wasdist_targ_disc)
            loss_seg.backward()
            self.dda_optim.step()
            ...
            disc_loss_cup = wasdist_src_cup - wasdist_targ_cup + gamma*regu_val_cup
            disc_loss_disc = wasdist_src_disc - wasdist_targ_disc + gamma*regu_val_disc
            disc_loss = beta_cup * disc_loss_cup + beta_disc * disc_loss_disc
            disc_loss.backward()
