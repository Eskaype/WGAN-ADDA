...
from trainer_dual_source.trainer_multi_source_adver import adversarial_madan_trainer
import torch
...
class adversarial:
    def __init__(self, args):
        ...
        self.attempt = 9.5
        self.adver_trainer = adversarial_madan_trainer(args, self.num_class, self.num_domains, loss_type)
        self.trainer_madadversarial(args)
    ...
    def save_model(self, epoch, IoU):
        print('---- MODEL SAVE ---')
        torch.save({'epoch': epoch + 1, 'state_dict': self.adver_trainer.target_model.state_dict(), 'best_auc': str(self.best_IoU['cup']),
                    'optimizer' : self.adver_trainer.dda_optim.state_dict()}, 'best_origa/m-adda_wgan_clip_0.03' + "v_" + str(self.attempt) + '.pth.tar')
        return

    def trainer_madadversarial(self, args):
        print("trainer initialized training started")
        for epoch in range(args.epochs):
            # self.validation(args, epoch)
            self.adver_trainer.generator_model.train()
            self.adver_trainer.discriminator_model.train()
            total_loss = 0
            ...
            options = {'gamma': 0.5 , 'mu': 1e-2, 'mode': 'maxmin', 'trainmodel': 'train_disc'}
            for step in trange(len_dataloader, leave=True):
                ...
                source_loss, tgt_loss = self.adver_trainer.update_weights_wasserstein(source_x, src_labels, target_x, target_lab, options) #0.2, 0.01
                total_loss+= tgt_loss
                if step %50 ==0:
    ...
    def validation(self, args, epoch):
        self.adver_trainer.generator_model.eval()
        test_loss = 0.0
        predict_disc =None
        target_disc = None
        predict_cup = None
        ...
        for i, data in enumerate(self.tbar):
            ...
            with torch.no_grad():
                output,_ = self.adver_trainer.generator_model(image)
            test_loss = self.adver_trainer.generator_criterion(output, target)
